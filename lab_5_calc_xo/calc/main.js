...
let simpleButtons = document.querySelectorAll('.print');
let cBut = document.querySelector('.clear');
let resultBut = document.querySelector('.result');
let mainSpan = document.querySelector('.input');
...
function evaluate(str) {
    ...
    for (let i = 0; i < all.length; i++) {
        ...
        if (isNumeric(curItem)) {
            stackNum.push(Number(curItem));
        } else if (isOperation(curItem)) {
            b = stackNum.pop();
            a = stackNum.pop();
            ...
            case '/':
                ...
                break;
            default:
                outputResult.textContent = "Неизвестная операция!";
                return; // Добавлен return после вывода сообщения об ошибке
            }
        } else {
            outputResult.textContent = "Ошибка ввода!";
            return; // Добавлен return после вывода сообщения об ошибке
        }
    }
...
// Функция, которую вы хотите выполнить при событии
function newChar(event) {
    let targ = event.target;
    let cont = targ.textContent;
    //console.log(targ.textContent);
    mainSpan.textContent += cont;
}
...
function start(event) {
    let task = mainSpan.textContent;
    mainSpan.textContent += '=';
    let readyForSolve = compile(task);
    //console.log(tokenize(readyForSolve));
    outputResult.textContent = evaluate(readyForSolve);
}

// Проходим массиву и добавляем обработчик ко всем элементам
for (let i = 0; i < simpleButtons.length; i++) {
    simpleButtons[i].addEventListener('click', newChar);

}
cBut.addEventListener('click', clearSpan);
resultBut.addEventListener('click', start);
